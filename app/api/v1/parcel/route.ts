/**
 * GET /api/v1/parcel — single-parcel lookup by street address or APN.
 *
 * Built 2026-09-18 for Solis "Parcel lookup" (Unlimited/Enterprise tiers). Solis's
 * server functions run on an edge runtime that cannot load snowflake-sdk, so Solis
 * calls this Node route with its own X-API-Key; Iconycs holds the warehouse creds.
 *
 ...
 *
 * Returns assessor/recorder fields ONLY: situs, characteristics, valuation/tax,
 * sale history, mortgage liens. Owner names, phones, HHID and household demographics
 * are deliberately excluded — this is a property record, not a people record.
 * Read-only against VW_RESIDENTIAL_PROP (residential parcels).
 */
...
export async function GET(request: NextRequest) {
  ...
  const apn = sp.get('apn')?.trim();
  const state = sp.get('state')?.trim().toUpperCase();
  const city = sp.get('city')?.trim().toUpperCase();
  ...
  if (apn) {
    mode = 'apn';
    if (!state) return errorResponse('state is required with apn (APN formats repeat across states)', 400);
    where.push(`v.STATE = ${q(state)}`);
    // Exact match first (assessor formatting, e.g. 080-509-000-0008 / 313-01-821). A normalized
    ...
    if (!parsed) return errorResponse('address must look like "2402 Windsor Ln"', 400);
    if (!zip && !(city && state)) return errorResponse('provide zip, or city + state, with address', 400);
    ...
    return errorResponse('provide address (+ zip or city/state) or apn (+ state)', 400);
  }
  ...
  try {
    ...
    if (!result.success) return errorResponse(result.error ?? 'Query failed');
